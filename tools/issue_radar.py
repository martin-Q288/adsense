...
INCIDENT = {"사고"}               # 피해자가 있는 사건 — "하는 법"류를 절대 붙이면 안 됨

AXES = {
    "방법/절차": [
        ("{k} 신청 방법", ADMIN), ("{k} 하는 법", {"정책", "금융", "생활", "제품", "리뷰", "시즌"}),
    ...
    "자격/조건": [
        ("{k} 자격 조건", ADMIN), ("{k} 대상자", ADMIN | INCIDENT),
    ...
    "오류/문제해결": [
        ...
        # 피해자 관점 — 가해/행위 동사가 아니라 확인·대응 동사만 쓴다
        ("{k} 확인 방법", INCIDENT), ("{k} 대처법", INCIDENT),
        ("{k} 대응 방법", INCIDENT), ("{k} 조회하는 법", INCIDENT),
    ],
    "연락처/창구": [
        ("{k} 고객센터", ADMIN | BUYABLE | INCIDENT), ("{k} 전화번호", ADMIN | INCIDENT),
        ("{k} 문의처", ADMIN | INCIDENT), ("{k} 접수처", ADMIN | INCIDENT),
        ("{k} 신고 방법", INCIDENT), ("{k} 위치", ADMIN),
    ],
    "준비물/서류": [
...
TYPE_PRIORITY = {
    ...
    "방송": ["시청/접속", "비교/대안"],
    "사고": ["오류/문제해결", "자격/조건", "연락처/창구", "계산/금액", "방법/절차"],
